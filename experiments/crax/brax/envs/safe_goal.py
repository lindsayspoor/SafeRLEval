...
class SafeGoal(PipelineEnv, ABC):
    ...
    def reset(self, rng: jp.ndarray) -> State:
        ...
        # Fixed hazards (e.g. boundary walls) keep their static XML-defined position,
        # but MJX does not initialize mocap_pos from the body's XML pos attribute, so
        # it defaults to (0, 0, 0) unless set explicitly here.
        fixed_hazards = self._hazard_manager.hazards[self._num_movable_hazards:]
        if fixed_hazards:
            fixed_hazard_ids = jp.array(
                self._hazard_mocap_ids[self._num_movable_hazards:], dtype=jp.int32
            )
            fixed_hazard_pos = jp.array(
                [h.position for h in fixed_hazards], dtype=jp.float32
            )
        else:
            fixed_hazard_ids = jp.array([], dtype=jp.int32)
            fixed_hazard_pos = jp.zeros((0, 3), dtype=jp.float32)

        # Only concatenate if we have movable hazards
        if self._num_movable_hazards > 0:
            all_ids = jp.concatenate([goal_ids, hazard_ids, fixed_hazard_ids])
            all_pos = jp.concatenate([goal_positions, hazard_positions, fixed_hazard_pos], axis=0)
        else:
            all_ids = jp.concatenate([goal_ids, fixed_hazard_ids])
            all_pos = jp.concatenate([goal_positions, fixed_hazard_pos], axis=0)
